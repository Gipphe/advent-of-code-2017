fn checksum(input: &str) -> u32 {
    ...
    for line in lines {
        ...
        for number in line.split_whitespace() {
            let number: u32 = number.parse().expect("Error parsing string to number");
            if number < min {
                min = number;
            }
            if number > max {
...
fn is_evenly_divisible(x: f64, y: f64) -> bool {
    x % y == 0.0 || y % x == 0.0
}
fn evenly_divide(x: u32, y: u32) -> u32 {
    if x < y {
        return y / x;
    }
    x / y
...
fn sum_evenly_divisible(input: &str) -> u32 {
    let lines = input.lines();
    let mut res: u32 = 0;
    'lines: for line in lines {
        let numbers_line = line.split_whitespace().collect::<Vec<&str>>();
        for i in 0..numbers_line.len() {
            let number = numbers_line[i].parse::<u32>().expect("Error parsing number");
            for j in (i + 1)..numbers_line.len() {
                let other_number = numbers_line[j].parse::<u32>().expect("Error parsing number");
                if is_evenly_divisible(number as f64, other_number as f64) {
                    res += evenly_divide(number, other_number);
                    continue 'lines;
                }
            }
        }
